 ...
 * {@link CustomItemTouchHelper.Callback#onMove(RecyclerView, RecyclerView.ViewHolder, RecyclerView.ViewHolder)} and / or
 * {@link CustomItemTouchHelper.Callback#onSwiped(RecyclerView.ViewHolder, int)}.
 * <p>
 ...
 * {@link CustomItemTouchHelper.Callback} class or implementing {@link CustomItemTouchHelper.ViewDropHandler}
 ...
 * overriding {@link CustomItemTouchHelper.Callback#onChildDraw(Canvas, RecyclerView, RecyclerView.ViewHolder, float, float, int,
 * boolean)}
 * or {@link CustomItemTouchHelper.Callback#onChildDrawOver(Canvas, RecyclerView, RecyclerView.ViewHolder, float, float, int,
 * boolean)}.
 * <p/>
 ...
 */
public class CustomItemTouchHelper extends RecyclerView.ItemDecoration
     ...
     * {@link CustomItemTouchHelper.Callback#getAbsoluteMovementFlags(RecyclerView, RecyclerView.ViewHolder)} for the current
     * action state.
     */
    int mSelectedFlags;
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    final CustomItemTouchHelper.RecoverAnimation animation = findAnimation(event);
                    if (animation != null) {
     ...
     */
    public CustomItemTouchHelper(CustomItemTouchHelper.Callback callback) {
        mCallback = callback;
    }
    ...
    private void destroyCallbacks() {
        ...
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final CustomItemTouchHelper.RecoverAnimation recoverAnimation = mRecoverAnimations.get(0);
    ...
    private void initGestureDetector() {
        ...
        mGestureDetector = new GestureDetectorCompat(mRecyclerView.getContext(),
                new CustomItemTouchHelper.ItemTouchHelperGestureListener());
    }
    ...
    void select(RecyclerView.ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                final CustomItemTouchHelper.RecoverAnimation rv = new CustomItemTouchHelper.RecoverAnimation(prevSelected, animationType,
    ...
    int endRecoverAnimation(RecyclerView.ViewHolder viewHolder, boolean override) {
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final CustomItemTouchHelper.RecoverAnimation anim = mRecoverAnimations.get(i);
     ...
     * {@link CustomItemTouchHelper.Callback#isLongPressDragEnabled()}.
     * <p>
     ...
     * ItemTouchHelper
     * is attached.</li>
     * <li>{@link CustomItemTouchHelper.Callback} must have dragging enabled.</li>
     ...
     *                   RecyclerView.
     * @see CustomItemTouchHelper.Callback#isItemViewSwipeEnabled()
     */
     ...
     * behavior
     * by overriding {@link CustomItemTouchHelper.Callback}
     * <p>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemTouchHelper is attached.</li>
     * <li>{@link CustomItemTouchHelper.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
        ...
        if (Math.abs(mDx) > Math.abs(mDy)) {
            if ((swipeDir = checkHorizontalSwipe(viewHolder, flags)) > 0) {
                ...
                if ((originalFlags & swipeDir) == 0) {
                    // convert to relative
                    return CustomItemTouchHelper.Callback.convertToRelativeDirection(swipeDir,
                            ViewCompat.getLayoutDirection(mRecyclerView));
                }
                return swipeDir;
            }
     ...
     * {@link CustomItemTouchHelper}.
     */
    public static interface ViewDropHandler {

        /**
         * Called by the {@link CustomItemTouchHelper} after a View is dropped over another View.
         * <p>
     ...
     * {@link CustomItemTouchHelper.SimpleCallback}.
     * <p>
    ...
    public abstract static class Callback {
        ...
        private static final CustomItemTouchUIUtil sUICallback;
        ...
        static {
            if (Build.VERSION.SDK_INT >= 21) {
                sUICallback = new CustomItemTouchUIUtilImpl.Lollipop();
            } else if (Build.VERSION.SDK_INT >= 11) {
                sUICallback = new CustomItemTouchUIUtilImpl.Honeycomb();
            } else {
                sUICallback = new CustomItemTouchUIUtilImpl.Gingerbread();
            }
        }

        /**
         * Returns the {@link CustomItemTouchUIUtilImpl} that is used by the {@link CustomItemTouchHelper.Callback} class for
         * visual
         * changes on Views in response to user interactions. {@link CustomItemTouchUIUtil} has different
         * implementations for different platform versions.
         * <p>
         * By default, {@link CustomItemTouchHelper.Callback} applies these changes on
         ...
         * </pre>
         *
         * @return The {@link ItemTouchUIUtil} instance that is used by the {@link CustomItemTouchHelper.Callback}
         */
        public static CustomItemTouchUIUtil getDefaultUIUtil() {
            return sUICallback;
        }
         ...
         *                    it is cleared.
         * @param actionState One of {@link CustomItemTouchHelper#ACTION_STATE_IDLE},
         *                    {@link CustomItemTouchHelper#ACTION_STATE_SWIPE} or
         *                    {@link CustomItemTouchHelper#ACTION_STATE_DRAG}.
        ...
        public void onMoved(final RecyclerView recyclerView,
            ...
            if (layoutManager instanceof CustomItemTouchHelper.ViewDropHandler) {
                ((CustomItemTouchHelper.ViewDropHandler) layoutManager).prepareForDrop(viewHolder.itemView,
                        target.itemView, x, y);
                return;
            }
        ...
        void onDraw(Canvas c, RecyclerView parent, RecyclerView.ViewHolder selected,
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                final CustomItemTouchHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
